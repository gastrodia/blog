...
tags:
  - Bun
ogImage: ""
description: 🎯端口被占用是开发中常见的问题。使用 Bun 创建跨平台命令行工具，支持 Windows、macOS、Linux 端口占用进程查找和终止，是 Bun 入门实战的绝佳项目。
---

## 思考

在我们使用有些npm库(cli)时，我们可以使用某些指令(命令行接口)来使用该库。例如`rollup`的`rollup src/main.js -f cjs`；或`vite`的`vite dev`。

那么他们时如何实现的？

关于`node`是实现该功能网上有很多文章，大家可自行查阅。

接下来我们使用`bun`来实现该功能。其实与nodejs实现原理一致。
我们来实现一个简单的关闭端口占用的工具;

## 创建项目

1. 新建`kill-port-bun`文件夹
